        ...
        <div className="p-4 flex flex-wrap gap-3 items-end">
          ...
          <div className="w-48">
            ...
            <Select value={warehouseType} onChange={(e) => { setWarehouseType(e.target.value); setPage(1); }}>
              {WAREHOUSE_TYPES.map(wt => <option key={wt.value} value={wt.value}>{wt.label}</option>)}
            </Select>
          </div>
          ...
          <Card key={wh.id}>
            ...
            >
              <div className="flex items-start justify-between">
                ...
                >
                  {(wh.available_space_sqm || wh.available_area_m2 || 0) > 0 ? t('common.active') : t('common.status')}
                </span>
              </div>
      ...
      >
        ...
        {(wh.available_space_sqm || wh.available_area_m2 || 0) > 0 && !booked && (
          <div className="border-t pt-4 space-y-3" style={{ borderColor: 'var(--ds-gray-300)' }}>
            <h3 className="text-sm font-bold" style={{ color: 'var(--ds-gray-1000)' }}>{t('warehouses.book')}</h3>
            ...
            <Button onClick={handleBook} disabled={submitting || !bookingFrom || !bookingTo || !bookingSpace}>
              {submitting ? t('common.loading') : t('warehouses.book')}
            </Button>
          </div>
        )}
